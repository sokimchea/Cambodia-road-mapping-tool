...
// Include all your other JavaScript functions here
		...
		function loadRoadSegment(roadNumber, startPK, endPK) {
			let road = geojsonData.features.find(f => f.properties.R_Number == roadNumber);
			if (!road) {
		...
		// for saving to KML files and merging to 1 files
		let allSegments = []; // Global array to store all segments before merging

		function saveAsKML(segment) {
			if (!segment || segment.length === 0) {
				alert("No road segment selected to save!");
				console.error("Error: No road segment data found.");
				return;
			}

			console.log("Adding segment to KML...");

			// Fetch input values
			let roadNameElement = document.getElementById("roadSearch");
			let startPKElement = document.getElementById("pkStart");
			let endPKElement = document.getElementById("pkEnd");

			if (!roadNameElement || !startPKElement || !endPKElement) {
				console.error("Error: One or more input fields not found.");
				alert("Error: Cannot retrieve road information.");
				return;
			}

			let roadName = roadNameElement.value.trim() || "Road_Segment";
			let startPK = startPKElement.value.trim() || "Start";
			let endPK = endPKElement.value.trim() || "End";

			if (!isNaN(startPK)) startPK = parseInt(startPK);
			if (!isNaN(endPK)) endPK = parseInt(endPK);

			roadName = roadName.replace(/\s+/g, "_").replace(/[^a-zA-Z0-9_]/g, "");
			startPK = startPK.toString().replace(/\D/g, "");
			endPK = endPK.toString().replace(/\D/g, "");

			let segmentName = `${roadName}_PK${startPK}_to_PK${endPK}`;

			// Store segment in global array
			allSegments.push({ segment, segmentName, roadName, startPK, endPK });

			console.log(`Segment ${segmentName} added. Total segments: ${allSegments.length}`);
			alert(`Segment ${segmentName} added! You can now add more roads before merging.`);
		}

		// Function to merge all saved KML segments and download as one file
		function mergeAndSaveKML() {
			if (allSegments.length === 0) {
				alert("No segments to save! Add at least one road segment first.");
				return;
			}
			...
			<kml xmlns="http://www.opengis.net/kml/2.2">
				<Document>
					<name>Merged_Road_Segments</name>`;

			allSegments.forEach(({ segment, segmentName, roadName, startPK, endPK }) => {
				kmlContent += `
					<!-- Start Point -->
					<Placemark>
						<name>${roadName} PK ${startPK} (Start)</name>
						<Style>
							<IconStyle>
								<scale>1.2</scale>
								<Icon>
						...
						<Point>
							<coordinates>${segment[0][0]},${segment[0][1]},0</coordinates>
						</Point>
					</Placemark>

					<!-- End Point -->
					<Placemark>
						<name>${roadName} PK ${endPK} (Finish)</name>
						<Style>
							<IconStyle>
								<scale>1.2</scale>
								<Icon>
						...
						<Point>
							<coordinates>${segment[segment.length - 1][0]},${segment[segment.length - 1][1]},0</coordinates>
						</Point>
					</Placemark>
			...
			// Save merged KML file
			try {
				const kmlBlob = new Blob([kmlContent], { type: 'application/vnd.google-earth.kml+xml;charset=utf-8' });
				let a = document.createElement("a");
				a.href = URL.createObjectURL(kmlBlob);
				a.setAttribute("download", "Merged_Road_Segments.kml");
				document.body.appendChild(a);
				a.click();
				document.body.removeChild(a);
				setTimeout(() => URL.revokeObjectURL(a.href), 1000);
				console.log("Merged KML file saved successfully.");
			} catch (error) {
				console.error("Error generating merged KML file:", error);
				alert("An error occurred while saving the merged KML file.");
			}
		}
		...
		//function to draw full line of maps
        ...
        fetch('test_V01.geojson') // UPDATE THIS PATH BASED ON YOUR FILE LOCATION
		...
		// Function to load road numbers from GeoJSON and populate the search list based on the input
		function loadRoadList(input) {
			...
			fetch('test_V01.geojson')
				...
				.then(data => {
					...
					// Filter and sort road numbers
					data.features
						.filter(feature => feature.properties.R_Number && feature.properties.R_Number.toLowerCase().includes(input.toLowerCase()))
						.sort((a, b) => a.properties.R_Number.localeCompare(b.properties.R_Number, undefined, {numeric: true})) // Sort numerically
						.forEach(feature => {
							roadNumbers.add(feature.properties.R_Number);
						});

					// Populate the datalist with sorted road numbers
				...
				.catch(error => console.error("Error loading or processing roads:", error));
		}

		// Event listener for input on the 'roadSearch' input field
		document.getElementById('roadSearch').addEventListener('input', function() {
			var input = this.value; // Get the current input value
			if (typeof input === 'string') {
				loadRoadList(input); // Call 'loadRoadList' with the current input value
		...
		function loadAndShowRoads() {
			fetch('test_V01.geojson') // Make sure the path to your GeoJSON file is correct
				.then(response => response.json())
				.then(data => {
					L.geoJSON(data, {
						...
						onEachFeature: function (feature, layer) {
							...
							layer.on('click', function () {
								alert("Road ID: " + feature.properties.R_Number); // Ensure the 'id' property exists in your GeoJSON
							});
							
							// Binding a tooltip to show the road number on hover
							if (feature.properties && feature.properties.R_Number) {
								layer.bindTooltip("Road Number: " + feature.properties.R_Number, {permanent: false, direction: "auto"});
							}
						}
					}).addTo(map);
				})
		...
		function exportToExcel() {
			// Assuming you have your GeoJSON data stored in a variable `geojsonData`
			// or you might fetch it as shown previously
			fetch('test_V01.geojson')
			.then(response => response.json())
			.then(data => {
				const roadData = data.features.map(feature => {
					const coordinates = normalizeCoordinate(feature.geometry.coordinates);
					const roadLength = computePKs(coordinates).totalDistance;
					return {
						'Road ID': feature.properties.FID,
						'Road Name': feature.properties.R_Number,
						'Length': roadLength.toFixed(2), // rounding to two decimal places for better presentation
						'PK Start': 0,
						'PK End': roadLength.toFixed(2)
					};
				});

				/* Using SheetJS to create workbook and add the JSON data */
				const ws = XLSX.utils.json_to_sheet(roadData);
				const wb = XLSX.utils.book_new();
				XLSX.utils.book_append_sheet(wb, ws, "Road Inventory");

				/* Generate buffer and prompt download */
				XLSX.writeFile(wb, "road_inventory.xlsx");
			})
			.catch(error => console.error('Error processing or exporting Excel:', error));
		}
